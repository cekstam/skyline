...
start () {
    ...
    /usr/bin/env python2.7 $BASEDIR/src/horizon/horizon-agent.py start
        RETVAL=$?
        if [[ $RETVAL -eq 0 ]]; then
...
stop () {
    ...
    /usr/bin/env python2.7 $BASEDIR/src/horizon/horizon-agent.py stop
        RETVAL=$?
        if [[ $RETVAL -eq 0 ]]; then
...
run () {
    ...
    /usr/bin/env python2.7 $BASEDIR/src/horizon/horizon-agent.py run
}
