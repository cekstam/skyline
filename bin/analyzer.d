...
start () {
    ...
    /usr/bin/env python2.7 $BASEDIR/src/analyzer/analyzer-agent.py start
        RETVAL=$?
        if [[ $RETVAL -eq 0 ]]; then
...
stop () {
    ...
    /usr/bin/env python2.7 $BASEDIR/src/analyzer/analyzer-agent.py stop
        RETVAL=$?
        if [[ $RETVAL -eq 0 ]]; then
...
run () {
    ...
    /usr/bin/env python2.7 $BASEDIR/src/analyzer/analyzer-agent.py run
}
